...
def test_clnrest_no_auto_start(node_factory):
    ...
    assert l1.daemon.is_in_log(r'plugin-clnrest(-rs)?(\.py)?: Killing plugin: disabled itself at init: `clnrest-port` option is not configured')
...
def test_clnrest_self_signed_certificates(node_factory):
    ...
    # This might happen really early!
    l1.daemon.logsearch_start = 0
    l1.daemon.wait_for_log(r'plugin-clnrest(-rs)?(\.py)?: REST server running at ' + base_url)
...
def test_clnrest_options(node_factory):
    ...
    rest_port = 1000
    l1 = node_factory.get_node(options={'clnrest-port': rest_port})
    assert l1.daemon.is_in_log(r'plugin-clnrest(-rs)?(\.py)?: '+ f'Killing plugin: disabled itself at init: `clnrest-port` {rest_port}, should be a valid available port between 1024 and 65535.')
    ...
    l1 = node_factory.get_node(options={'clnrest-port': rest_port,
                                        'clnrest-protocol': rest_protocol})
    assert l1.daemon.is_in_log(r'plugin-clnrest(-rs)?(\.py)?: Killing plugin: disabled itself at init: `clnrest-protocol` can either be http or https.')
    ...
    l1 = node_factory.get_node(options={'clnrest-port': rest_port,
                                        'clnrest-host': rest_host})
    assert l1.daemon.is_in_log(r'plugin-clnrest(-rs)?(\.py)?: Killing plugin: disabled itself at init: `clnrest-host` should be a valid IP.')
...
def test_clnrest_http_headers(node_factory):
    ...
    # This might happen really early!
    l1.daemon.logsearch_start = 0
    l1.daemon.wait_for_log(r'plugin-clnrest(-rs)?(\.py)?: ' + f'REST server running at {base_url}')
    ...
    l2.daemon.wait_for_log(r'plugin-clnrest(-rs)?(\.py)?: ' + f'REST server running at {base_url}')
    ca_cert = Path(rest_certs) / 'ca.pem'
